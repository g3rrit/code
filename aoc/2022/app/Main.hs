...
data Days
  = AllDays
  | OneDay Int (Maybe String)
  deriving (Show)

type Verbose = Bool

data Options = Options Days Verbose
...
days :: Map.Map Int (Bool -> String -> IO (), String)
days =
  Map.fromList $
    zip
      [1 ..]
...
performDay (Options ds v) = case ds of
  AllDays ->
    mapM_
      ( \(day, (a, i)) -> do
          putStrLn $ "\n***Day " <> printf "%02d" day <> "***"
...
main :: IO ()
main = execParser opts >>= performDay
  where
    opts =
      info
